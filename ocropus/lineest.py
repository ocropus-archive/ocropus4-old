...
class CenterNormalizer:
    ...
    def measure(self, line):
        h, w = line.shape
        smoothed = filters.gaussian_filter(
            line, (h * 0.5, h * self.smoothness), mode="constant"
        )
        smoothed += 0.001 * filters.uniform_filter(
            smoothed, (h * 0.5, w), mode="constant"
        )
        self.shape = (h, w)
        ...
        if np.isnan(self.range) or np.isnan(self.mad):
            self.r = h // 2
        else:
    ...
    def dewarp(self, img, cval=0, dtype=np.dtype("f")):
        ...
        padded = np.vstack(
            [cval * np.ones((hpadding, w)), img, cval * np.ones((hpadding, w))]
        )
    ...
    def normalize(self, img, order=1, dtype=np.dtype("f"), cval=0):
        ...
        scaled = scale_to_h(
            dewarped, self.target_height, order=order, dtype=dtype, cval=cval
        )
        return scaled

    def measure_and_normalize(self, img, order=1, dtype=np.dtype("f"), cval=0):
        self.measure(img)
        return self.normalize(img, order=order, dtype=dtype, cval=cval)
