...
def parse_param(s):
    ...
    if s == "[]" or s.lower() == "empty":
        return []
    if "," in s:
        return map(parse_param, s.split(","))
    return s
...
def make(name, *args, device=default_device, **kw):
    lst = name.split(";")
    name = lst[0]
    params = {r[0]: parse_param(r[1]) for r in map(lambda x: x.split("=", 1), lst[1:])}
    kw.update(params)
    print("# making ", name, kw)
...
def make_lstm_simple(noutput=noutput, height=48):
    model = nn.Sequential(
        layers.Input("BDHW", range=(0, 1), sizes=[None, 1, height, None]),
        # *combos.conv2d_block(100, 3, mp=2),
        *project_and_lstm(100, noutput),
    )
    flex.shape_inference(model, (1, 1, height, 512))
    return model
...
def make_lstm_normalized(noutput=noutput):
    ...
    flex.shape_inference(model, (1, 1, 48, 200))
    return model
...
def make_seg_lstm(noutput=3, sizes=[16, 32, 64], lstms=100, post=20, full=10, repeat=3):
    blocks = []
    for size in mklist(sizes):
        blocks += combos.conv2d_block(size, 3, mp=2, repeat=repeat)
    for size in mklist(lstms):
        blocks += [flex.BDHW_LSTM(size)]
    for size in mklist(post):
        blocks += combos.conv2d_block(size, 3)
    conv_lstm_block = nn.Sequential(*blocks)
    model = nn.Sequential(
        layers.Input("BDHW", range=(0, 1), sizes=[None, 1, None, None]),
        layers.KeepSize(sub=conv_lstm_block),
        *[item for item in combos.conv2d_block(size, 5) for size in mklist(full)],
        flex.Conv2d(noutput, 3),
    )
    flex.shape_inference(model, (1, 1, 256, 256))
    return model
