...
def extract(image, y0, x0, y1, x1, mode="nearest", cval=0):
    ...
    try:
        ...
        if cw > w or ch > h:
            ...
            r = interpolation.affine_transform(
                r, eye(2), offset=(pady0, padx0), cval=1, output_shape=(ch, cw)
            )
        return r
...
def extract_masked(image, linedesc, pad=5, expand=0, background=None):
    ...
    y0, x0, y1, x1 = [
        int(x)
        for x in [
            linedesc.bounds[0].start,
            linedesc.bounds[1].start,
            linedesc.bounds[0].stop,
            linedesc.bounds[1].stop,
        ]
    ]
    if pad > 0:
...
def hysteresis_threshold(image, lo, hi):
    ...
    n += 1
    good = unique((lablo * (image > hi)).flat)
    markers = zeros(n, "i")
    for index in good:
        if index == 0:
            continue
        markers[index] = 1
    result = markers[lablo]
    assert result.shape == image.shape, (result.shape, image.shape)
    return result
...
class LineSegmenter(object):
    ...
    def line_probs(self, pimage):
        ...
        oh, ow = poutput.shape
        scale = oh * 1.0 / ih
        poutput = ndi.affine_transform(
            poutput, eye(2) * scale, output_shape=pimage.shape, order=1
        )
        self.probs = poutput
        return poutput
